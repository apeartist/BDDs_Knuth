...
int ZDD::solutions() {
	...
	st.push(rootIndex);
	while (!st.empty()) {
		Node cur = nodes[st.top()];
		st.pop();
		// hi
		if (nodes[cur.hi].val == vmax) {
			if (nodes[cur.hi].lo == 1) {
		...
		// lo
		if (nodes[cur.lo].val == vmax) {
			if (nodes[cur.lo].lo == 1) {
				// check if the node is True
				out++;
			}
		}
...
void ZDD::reduce() {
	...
	for (int v = vmax; v >= nodes[rootIndex].val; v--) {
		...
		while (p != 0) {
			...
			if (nodes[p].hi == 0) { // if hi is false
				nodes[p].lo = ~nodes[p].lo;
				nodes[p].hi = avail;
				aux[p] = 0;
				avail = p;
			}
